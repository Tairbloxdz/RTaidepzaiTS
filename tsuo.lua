...
object.taidepzaiHub = instance.new("ScreenGui", {
    Name = "Mo Mo Be";
    Parent = game.CoreGui;
...
pcall(function()
    if true and isfile('taidepzai.bin') and not getgenv().Key then
        object.TextBox.Text = readfile("taidepzai.bin")
...
local function SaveKey()
    local s, e = pcall(function()
        if not isfile('taidepzai.bin') then
            writefile('taidepzai.bin', GetKeyText())
        end
    end)
end
...
local function RemoveKey()
    local s, e = pcall(function()
        if isfile('taidepzai.bin') then
            delfile('taidepzai.bin')
        end
    end)
end
...
local URL = 'https://taidepzai-script.xyz/'
...
local function wl(key)
    ...
    if sign_match == decode_response.sign then
        ...
        local Window =
            Fluent:CreateWindow(
            {
                Title = "taidepzai Hub Update 20",
                SubTitle = "Dev By Duytai coder",
                TabWidth = 160,
        ...
        function Window:NewTab(mmb)
            ...
            function newmm:NewParagraph(cf)
                local paragraphnew
                newTitle = cf.Title or "taidepzai Hub"
                ...
                function paragraphnew:Set(cf2)
                    newTitle = cf2.Title or "taidepzai Hub"
        ...
        function SaveSettings2()
            local HttpService = game:GetService("HttpService")
            if not isfolder("taidepzai Hub") then
                makefolder("taidepzai Hub")
            end
            writefile("taidepzai Hub/" .. SaveFileName2, HttpService:JSONEncode(Settings2))
        end
        ...
        function ReadSetting2()
            local s, e =
                ...
                function()
                    ...
                    return HttpService:JSONDecode(readfile("taidepzai Hub/" .. SaveFileName2))
                end
            )
            if s then
                return e
